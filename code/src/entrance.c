...
EntranceName entranceNames[] = {
    { 0x00BB, "Child Spawn" , "KF Links House" , ENTRANCE_GROUP_ROOT_EXITS },
    { 0x0282, "Adult Spawn" , "Temple of Time" , ENTRANCE_GROUP_ROOT_EXITS },
    { 0x0600, "Minuet of Forest" , "SFM" , ENTRANCE_GROUP_ROOT_EXITS },
    { 0x04F6, "Bolero of Fire" , "DMC Warp Pad" , ENTRANCE_GROUP_ROOT_EXITS },
    { 0x0604, "Serenade of Water" , "Lake Hylia" , ENTRANCE_GROUP_ROOT_EXITS },
    { 0x01F1, "Requiem of Spirit" , "Desert Colossus" , ENTRANCE_GROUP_ROOT_EXITS },
    { 0x0568, "Nocturne of Shadow" , "Graveyard Warp Pad" , ENTRANCE_GROUP_ROOT_EXITS },
    { 0x05F4, "Prelude of Light" , "Temple of Time" , ENTRANCE_GROUP_ROOT_EXITS },
    ...
    { 0x027E, "LH Owl Flight" , "Hyrule Field" , ENTRANCE_GROUP_LAKE_HYLIA },
    { 0x0554, "DMT Owl Flight" , "Kakariko Village" , ENTRANCE_GROUP_DEATH_MOUNTAIN_TRAIL },
};
...
char* GetEntranceName(s16 index_, u8 type) {
    for (size_t i = 0; i < sizeof(entranceNames) / sizeof(EntranceName); i++) {
        if (index_ == entranceNames[i].index) {
            if (type == ENTRANCE_NAME_SOURCE) {
                return entranceNames[i].source;
            } else if (type == ENTRANCE_NAME_DESTINATION) {
                return entranceNames[i].destination;
            }
        }
    }
    return NULL;
...
void InitEntranceTrackingData(void) {
    ...
    for (size_t i = 0; i < sizeof(entranceNames) / sizeof(EntranceName); i++) {
        if (entranceNames[i].source == NULL) {
            break;
        }
