...
EntranceData entranceData[] = {
    ...
    { 0x00BB, "Child Spawn" , "KF Links House" , ENTRANCE_GROUP_ROOT_EXITS, ENTRANCE_TYPE_SPAWN },
    { 0x0282, "Adult Spawn" , "Temple of Time" , ENTRANCE_GROUP_ROOT_EXITS, ENTRANCE_TYPE_SPAWN },

    { 0x0600, "Minuet of Forest" , "SFM" , ENTRANCE_GROUP_ROOT_EXITS, ENTRANCE_TYPE_WARP_SONG },
    { 0x04F6, "Bolero of Fire" , "DMC Warp Pad" , ENTRANCE_GROUP_ROOT_EXITS, ENTRANCE_TYPE_WARP_SONG },
    { 0x0604, "Serenade of Water" , "Lake Hylia" , ENTRANCE_GROUP_ROOT_EXITS, ENTRANCE_TYPE_WARP_SONG },
    { 0x01F1, "Requiem of Spirit" , "Desert Colossus" , ENTRANCE_GROUP_ROOT_EXITS, ENTRANCE_TYPE_WARP_SONG },
    { 0x0568, "Nocturne of Shadow" , "Graveyard Warp Pad" , ENTRANCE_GROUP_ROOT_EXITS, ENTRANCE_TYPE_WARP_SONG },
    { 0x05F4, "Prelude of Light" , "Temple of Time" , ENTRANCE_GROUP_ROOT_EXITS, ENTRANCE_TYPE_WARP_SONG },
};
...
void SortEntranceList(EntranceOverride* entranceList, u8 byDest) {
    ...
    // Sort Destination List
        ...
        for (size_t i = 0; i < ENTRANCE_OVERRIDES_MAX_COUNT; i++) {
            EntranceData* curEntrance = GetEntranceData(entranceList[i].index);
            if (curEntrance->group != ENTRANCE_GROUP_ROOT_EXITS) {
                for (size_t j = 0; j < ENTRANCE_OVERRIDES_MAX_COUNT; j++) {
                    EntranceData* curOverride = GetEntranceData(tempList[j].override);
                    if (strcmp(curEntrance->source, curOverride->destination) == 0 &&
                       (strcmp(curEntrance->destination, curOverride->source) == 0 &&
                        GetEntranceData(tempList[j].index)->group != ENTRANCE_GROUP_ROOT_EXITS)) {
                           entranceList[i] = tempList[j];
                           break;
                    }
                }
            }
